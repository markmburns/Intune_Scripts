#Detection to determine if unwanted machine certs exist
...
foreach($cert IN $myMachineCerts){
    ...
    If($templateName -match $intendedTemplate){
        If($cert.Subject -match $intendedSubject){
            ...
            $validCertFound = $true
            $output = "$($output) | Valid: $($cert.Subject), $($templateName)"
        }
        If($cert.Subject -notmatch $intendedSubject){
            #Write-Output "Extraneous certificate found: $($cert.Subject), $($templateName)"
            $extraneousCertFound = $true
            $output = "$($output) | Extraneous: $($cert.Subject), $($templateName)"
        }
    }
}
If($validCertFound -and $extraneousCertFound){
    Write-Output "Valid and extraneous certs found.$($output)"
    exit 1
}else{
    Write-Output "Valid and extraneous certs not found.$($output)"
    exit 0
}
