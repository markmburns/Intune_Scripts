...
Version 1.4: IM logic

.DESCRIPTION 
Rename the computer, using IM logic
...
if (-not (Test-Path "$($env:ProgramData)\RenameComputer-IM"))
{
    Mkdir "$($env:ProgramData)\RenameComputer-IM"
}
Set-Content -Path "$($env:ProgramData)\RenameComputer-IM\RenameComputer-IM.ps1.tag" -Value "Installed"

# Initialization
$dest = "$($env:ProgramData)\RenameComputer-IM"
if (-not (Test-Path $dest))
{
    mkdir $dest
}
Start-Transcript "$dest\RenameComputer-IM.log" -Append

#Have we already run successfully?
If (Test-Path "$($env:ProgramData)\RenameComputer-IM\Renamed.tag"){
...
if ($goodToGo)
{
    ...
    Set-Content -Path "$($env:ProgramData)\RenameComputer-IM\Renamed.tag" -Value "Installed"
...
else
{
    ...
    if (-not (Test-Path "$dest\RenameComputer-IM.ps1"))
    {
        Copy-Item $PSCommandPath "$dest\RenameComputer-IM.PS1"
    }

    # Create the scheduled task action
    $action = New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "-NoProfile -ExecutionPolicy bypass -WindowStyle Hidden -File $dest\RenameComputer-IM.ps1"
