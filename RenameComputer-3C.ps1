...
Version 1.4: 3C logic

.DESCRIPTION 
Rename the computer, using 3C logic
...
if (-not (Test-Path "$($env:ProgramData)\RenameComputer-3C"))
{
    Mkdir "$($env:ProgramData)\RenameComputer-3C"
}
Set-Content -Path "$($env:ProgramData)\RenameComputer-3C\RenameComputer-3C.ps1.tag" -Value "Installed"

# Initialization
$dest = "$($env:ProgramData)\RenameComputer-3C"
if (-not (Test-Path $dest))
{
    mkdir $dest
}
Start-Transcript "$dest\RenameComputer-3C.log" -Append

#Have we already run successfully?
If (Test-Path "$($env:ProgramData)\RenameComputer-3C\Renamed.tag"){
...
if ($goodToGo)
{
    ...
    Set-Content -Path "$($env:ProgramData)\RenameComputer-3C\Renamed.tag" -Value "Installed"
...
else
{
    ...
    if (-not (Test-Path "$dest\RenameComputer-3C.ps1"))
    {
        Copy-Item $PSCommandPath "$dest\RenameComputer-3C.PS1"
    }

    # Create the scheduled task action
    $action = New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "-NoProfile -ExecutionPolicy bypass -WindowStyle Hidden -File $dest\RenameComputer-3C.ps1"
