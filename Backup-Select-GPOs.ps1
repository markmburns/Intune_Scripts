#mark.burns@dell.com
#Backs up select GPOs in a domain, ready for Group Policy Analytics Import
#Variables
$GPOBackups = "C:\GPOBackups"
$backupGPOsFile = "BackupGPOs.txt"

#Generate array from input file
$BackupGPOs = Get-Content -Path "$PSScriptRoot\$backupGPOsFile"
$BackupGPOs = $BackupGPOs | Select-Object -Unique
...
foreach ($gpo in $gpos) {
# Write-Host "Looking for $($gpo.DisplayName) in backupGPOs"
 If($BackupGPOs -contains $gpo.DisplayName){
